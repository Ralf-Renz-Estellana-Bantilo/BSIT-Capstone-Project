...
const JobPosts = ({
	...
	const [isModalOpenClosePost, setisModalOpenClosePost] = useState(false);
	...
	const handleDeletePost = async () => {
		const { JobID, CompanyID } = postPreview;
		try {
			await axios
				.delete(`${AppConfiguration.url()}/api/delete-jobPost`, {
					companyID: CompanyID,
					jobID: JobID,
				})
				.then((response) => {});
			await axios
				.delete(`${AppConfiguration.url()}/api/delete-job-applicants`, {
					companyID: CompanyID,
					jobID: JobID,
				})
				.then((response) => {});
			await axios
				.delete(`${AppConfiguration.url()}/api/delete-applied-job`, {
					companyID: CompanyID,
					jobID: JobID,
				})
				.then(async (response) => {
	...
	return (
		<>
			...
			<div className='job-post-container'>
				...
				{isModalOpenClosePost && (
					<Modal
						headText='Close Post Confirmation'
						modalText={`Are you sure you wanna close this job post?`}
						confirmText='Yes'
						closeText='No'
						close={() => setisModalOpenClosePost(false)}
						confirm={handleClosePost}
						path='/admin/job-posts'
					/>
				)}
